...
//       let response;
//       if (isEditing) {
//         response=await axios.put(`http://localhost:3000/product/editproduct/${form.id}`, formData,{
//           headers: {
...
//       } else {
//         response=await axios.post("http://localhost:3000/product/addproduct", formData,{
//           headers: {
//             'Content-Type': 'multipart/form-data', // Needed for file uploads
//           }
//         });
...
//         {isModalOpen && (
//           <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 text-black px-4 sm:px-0">
...
//                 <input type="text" name="name" placeholder="Name" value={form.name} onChange={handleInputChange} className="w-full p-2 rounded" required />
//                 <input type="number" name="price" placeholder="Price" value={form.price} onChange={handleInputChange} className="w-full p-2 rounded" required />
//                 <input type="number" name="originalPrice" placeholder="Original Price" value={form.originalPrice} onChange={handleInputChange} className="w-full p-2 rounded" required />
//                 <input type="number" name="stock" placeholder="Stock" value={form.stock} onChange={handleInputChange} className="w-full p-2 rounded" required />
//                 <input type="number" name="categoryId" placeholder="Category Id" value={form.categoryId} onChange={handleInputChange} className="w-full p-2 rounded" required />
//                 <textarea name="description" placeholder="Description" value={form.description} onChange={handleInputChange} className="w-full p-2 rounded" required></textarea>
//                 <input type="file" onChange={handleFileChange} className="w-full p-2 rounded" />
//                 <div className="flex justify-between mt-4">
//                   <button type="submit" className="bg-blue-500 px-4 py-2 rounded text-white">{isEditing ? "Update" : "Add"}</button>
//                   <button onClick={() => setIsModalOpen(false)} className="bg-red-500 px-4 py-2 rounded text-white">Cancel</button>
//                 </div>
//               </form>
//             </div>
//           </div>
...
const Products = () => {
  ...
  console.log("Editing Product ID:", form.id);
      ...
      <div className="p-4 sm:p-6 bg-gray-900 text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            ...
            >
              <img
                src={product.images && product.images.length > 0 ? `${API_URL}/${JSON.parse(product.images)[0]}` : "default-image-path.jpg"}
                alt={product.name}
                className="w-full h-32 sm:h-40 object-cover rounded-xl"
              />
